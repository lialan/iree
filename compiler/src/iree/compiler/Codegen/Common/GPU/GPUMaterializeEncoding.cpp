...
struct GPUUnsetEncodingOpLoweringConversion
  ...
  LogicalResult
  matchAndRewrite(IREE::Encoding::UnsetEncodingOp unsetEncodingOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto converter = static_cast<const MaterializeEncodingTypeConverter *>(
        getTypeConverter());
    ...
    // From:
    // unpackOpSource -> unpackOp
    // To:
    // unpackOpSource -> expand_shape -> transpose -> collapse_shape -> unpack 
    // Dimensions:
    // unpackOpSource -> 4D

    // collapse_shape: 4D -> 4D
    // expand_shape: 4D -> 6D
